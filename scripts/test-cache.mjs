...
// ── 2bis. les hôtels : même correction, plus gros poste (étape 3) ──
// L'audit du 25 août a mesuré 93 Mo par passage complet des robots sur
// cette seule famille — dix-huit fois /destinations. Elle était classée
// après lui parce qu'on comparait le poids d'UNE page au lieu du poids de
// la famille.
const H_FR = fichierRoute('hotels/[ville]/page.tsx')
const H_EN = 'app/en/hotels/[ville]/page.tsx'
const H_RENDU = 'components/hotels/HotelsRoute.tsx'
for (const f of [H_FR, H_EN, H_RENDU]) {
  if (!existsSync(f)) casse(`${f} manque : la page hôtels n'a plus ses deux langues`)
}
if (existsSync(H_RENDU)) {
  const rh = readFileSync(H_RENDU, 'utf8').replace(/^\s*\/\/.*$/gm, '')
  for (const [motif, quoi] of [[/getDomainSEO/, 'getDomainSEO'], [/\bheaders\(\)/, 'headers()']]) {
    if (motif.test(rh)) casse(`la page hôtels lit encore ${quoi} : 93 Mo repartiraient du serveur à chaque passage`)
  }
  // 🔴 JAMAIS D'AVIS INVENTÉS. Mesuré le 25 août : aucun des 33 322 hôtels
  // de la base ne porte de nombre d'avis réel, et 3 532 annonçaient malgré
  // tout « 20 avis » à Google en données structurées.
  if (/ratingCount:\s*[^\n]*\?\?\s*\d/.test(rh)) {
    casse('la page hôtels réinvente un nombre d\'avis par défaut : c\'est une preuve fabriquée, pas une approximation')
  }
  if (/aggregateRating/.test(rh) && !/Number\(h\.avis_count\) > 0/.test(rh)) {
    casse('la note d\'un hôtel peut être publiée sans nombre d\'avis réel')
  }
}
if (existsSync(H_FR) && !/en={false}/.test(readFileSync(H_FR, 'utf8'))) casse('la route hôtels française ne force plus le français')
if (existsSync(H_EN) && !/en={true}/.test(readFileSync(H_EN, 'utf8'))) casse('la route hôtels anglaise ne force plus l\'anglais')
...
for (const fam of ['destinations', 'hotels']) {
  if (!new RegExp(`\\(destinations\\|hotels\\)|\\b${fam}\\b`).test(mw)) {
    casse(`le middleware ne réécrit plus /${fam}/... vers son chemin anglais`)
  }
}
