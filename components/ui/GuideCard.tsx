  ...
  return (
    <Link href={`${basePath}/${guide.slug}`} className="group block">
      <div className="bg-white rounded-2xl overflow-hidden shadow-sm border border-gray-100 hover:shadow-lg hover:-translate-y-1 transition-all duration-300">
        <div className="bg-gradient-to-br from-gray-100 to-gray-200 h-44 relative flex items-center justify-center">
          <span className="text-5xl opacity-50">📖</span>
          <div className="absolute top-3 left-3">
            <span className={`text-xs font-semibold px-2.5 py-1 rounded-full ${colorClass}`}>{guide.category}</span>
          </div>
        </div>
        ...
        <div className="p-5">
          <div className="flex items-center gap-2 text-xs text-gray-400 mb-2">
            ...
            <span>·</span>
            <span>{new Date(guide.publishedAt).toLocaleDateString('fr-FR', { year: 'numeric', month: 'long' })}</span>
          </div>
          <h3 className="font-bold text-gray-900 group-hover:text-emerald-600 transition-colors line-clamp-2">{guide.title}</h3>
