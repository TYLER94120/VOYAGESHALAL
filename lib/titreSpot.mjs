...
//
// ⚠️ POURQUOI CE FICHIER EST EN .mjs ET NON EN .ts. Le 14 août, le
// garde-fou de ces titres a été branché sur `npm run build` avec
// `node --experimental-strip-types`, qui permet d'importer un .ts
// directement. Ce drapeau n'existe qu'à partir de Node 22.6 — la
// plateforme de déploiement, elle, tourne sur une version antérieure.
// Résultat : **cinq déploiements de production échoués d'affilée**, et
// cinq heures pendant lesquelles les corrections livrées ne sont jamais
// parties en ligne. Le correctif était bon, il n'arrivait simplement
// jamais chez le visiteur.
//
// En JavaScript pur, le fichier s'importe partout : par l'application
// (allowJs est actif), par le test, et par n'importe quelle version de
// Node. Un garde-fou ne doit jamais dépendre d'un drapeau expérimental.
//
...
export function contientDuFrancais(texte) {
 ...
 */
export function replier(versions, max = TITRE_MAX) {
 ...
 */
export function titreSpot({ nom, villeNom, marque, isEN, typeLieuEn }) {
  if (!isEN) {
    const versions = [
...
export function descriptionSpot({ nom, villeNom, lieu, isEN }) {
  if (!isEN) {
    const versions = [
