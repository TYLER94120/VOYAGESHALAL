...
import { accord } from '@/lib/accordFr.mjs'
...
export function buildVilleFaq(ville: Ville, en: boolean): FaqItem[] {
  ...
  // 🔢 5 septembre : l'accord passe par le service commun (lib/accordFr.mjs).
  // Il vivait ICI et nulle part ailleurs — d'où « 1 mosquées » dans le TITRE
  // de 28 villes, dans les deux langues, pendant que cette FAQ écrivait
  // correctement « 1 mosque ». Une règle vraie à un endroit et fausse à
  // l'autre : la même forme de défaut six nuits d'affilée.
  const plEn = (n: number, s: string) => `${n.toLocaleString('en-US')} ${accord(n, `${s}s`)}`
  const plFr = (n: number, s: string) => `${n.toLocaleString('fr-FR')} ${accord(n, `${s}s`)}`
  ...
  if (en) {
    ...
    if (restoCount > 0) items.push({ q: `Are there many halal restaurants in ${nom}?`, a: `We list ${restoCount.toLocaleString('en-US')} halal ${accord(restoCount, 'restaurants')}${topRestos.length ? `, including ${topRestos.join(', ')}` : ''}. Each listing shows its location and a halal-confidence level.` })
  ...
  if (restoCount > 0) items.push({ q: `Y a-t-il beaucoup de restaurants halal ${aVille(nom)} ?`, a: `Nous listons ${restoCount.toLocaleString('fr-FR')} ${accord(restoCount, 'restaurants halal')}${topRestos.length ? `, dont ${topRestos.join(', ')}` : ''}. Chaque adresse indique sa localisation et un niveau de confiance halal.` })
