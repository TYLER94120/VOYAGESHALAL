...
import { aVille } from './prepositionVille.mjs'
import { accord } from './accordFr.mjs'
...
export function titresVilleEn(nom, nbPriere, sourceOsm) {
  ...
  const plafonne = !sourceOsm && nbPriere >= PLAFOND_MOSQUEES
  ...
  // 5 septembre : accordé au nombre — « 1 mosques » sortait sur 28 villes.
  const quoi = accord(nbPriere, sourceOsm ? 'prayer places' : 'mosques', plafonne)
  return [
...
export function titresVilleFr(nom, nbPriere, sourceOsm) {
  if (!nbPriere || nbPriere <= 0) {
    return [`Où prier et manger halal ${aVille(nom)}`, `Voyager halal ${aVille(nom)}`]
  }
  const plafonne = !sourceOsm && nbPriere >= PLAFOND_MOSQUEES
  const n = sourceOsm ? nb(nbPriere).replace(/,/g, ' ') : compte(nbPriere, PLAFOND_MOSQUEES).replace(/,/g, ' ')
  // 5 septembre : accordé au nombre. « 1 mosquées » et « 1 lieux de prière »
  // sortaient sur 28 villes — dans le titre.
  const quoi = accord(nbPriere, sourceOsm ? 'lieux de prière' : 'mosquées', plafonne)
  ...
  return [
    `Où prier ${aVille(nom)} : ${n} ${quoi} et les restos halal`,
    `Où prier ${aVille(nom)} : ${n} ${quoi}, restos halal`,
    `Où prier ${aVille(nom)} : ${n} ${quoi}`,
    `Où prier ${aVille(nom)}`,
  ]
}
...
export function descriptionVille({ nom, nbRestos, nbHotels, en }) {
  const bouts = []
  // Accordés eux aussi : une seule adresse ou un seul hôtel arrive, et la
  // description est la deuxième ligne que Google affiche.
  if (nbRestos > 0) bouts.push(`${compte(nbRestos, PLAFOND_RESTOS)} ${accord(nbRestos, en ? 'halal places to eat' : 'adresses halal', nbRestos >= PLAFOND_RESTOS)}`)
  if (nbHotels > 0) bouts.push(`${compte(nbHotels, PLAFOND_HOTELS)} ${accord(nbHotels, en ? 'hotels' : 'hôtels', nbHotels >= PLAFOND_HOTELS)}`)
  ...
  const texte = en
    ...
    : `${liste ? `${liste} ${aVille(nom)}, ` : ''}horaires de prière au fuseau de la ville. Source affichée sur chaque adresse. Gratuit, sans compte.`
