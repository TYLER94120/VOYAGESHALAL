...
/**
 * Ce lieu porte-t-il un marqueur d'alcool ? Sert à MESURER (combien
 * d'établissements de nos relevés en portent), jamais à condamner un lieu.
 * Les « juice bar » et autres sont neutralisés comme ailleurs.
 */
export function aMarqueurAlcool(nom, type) {
  const texte = `${nom ?? ''} ${type ?? ''}`.replace(BARS_SANS_ALCOOL, ' ')
  return MOTIF_ALCOOL.test(texte)
}
