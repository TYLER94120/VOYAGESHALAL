...
/** Le mot SEULEMENT s'il est spécifique (niveau 2 de la chaîne de
 *  fiabilité) — null quand le type Google est générique : c'est alors à
 *  l'IA (niveau 3, sur les avis) ou au générique honnête de parler. */
export function motSpecifique(famille, en = false) {
  const GENERIQUES = new Set(['restaurant', 'meal_takeaway', 'meal_delivery', 'tourist_attraction'])
  if (!famille || GENERIQUES.has(famille)) return null
  const m = MOTS[famille]
  return m ? m[en ? 1 : 0] : null
}
