    ...
    <main style={{ backgroundColor: '#faf8f4' }}>
      <section className="min-h-[90vh] grid grid-cols-1 lg:grid-cols-2">
        <div style={{ backgroundColor: '#faf8f4' }} className="flex flex-col justify-center px-8 sm:px-16 lg:px-20 py-20 lg:py-32">
          <div className="flex items-center gap-3 mb-8">
            ...
            <span style={{ color: '#c9a870' }} className="text-xs font-semibold uppercase tracking-[0.2em]">Le guide de référence mondial</span>
          </div>
          <h1 className="text-5xl sm:text-6xl lg:text-7xl font-bold leading-[1.05] mb-8" style={{ fontFamily: 'var(--font-playfair), Georgia, serif', color: '#1a3a2a' }}>
            Voyagez halal,<br />voyagez <em style={{ color: '#c9a870', fontStyle: 'italic' }}>serein</em>
          </h1>
          ...
          <form action="/search" method="GET" className="flex gap-0 max-w-md">
            <div className="relative flex-1">
              ...
              <input type="text" name="q" placeholder="Istanbul, Marrakech, Dubaï..." autoComplete="off"
            ...
            <button type="submit" style={{ backgroundColor: '#1a3a2a' }} className="text-white px-7 py-4 rounded-r-2xl font-semibold text-base hover:opacity-90 transition-opacity whitespace-nowrap">Rechercher</button>
          </form>
          <div className="mt-8 flex gap-8">
            {[{ value: '50+', label: 'destinations' }, { value: '3', label: 'continents' }, { value: '4 ans', label: "d'expérience" }].map((s) => (
              <div key={s.label}>
        ...
        <div className="relative hidden lg:block">
          <Image src="https://images.unsplash.com/photo-1524231757912-21f4fe3a7200?w=1200&q=85" alt="Istanbul — Voyages Halal" fill className="object-cover" priority />
        </div>
      </section>
        ...
        <div className="max-w-5xl mx-auto">
          <div className="flex items-end justify-between mb-10">
            <div>
              ...
              <h2 className="text-3xl font-bold" style={{ fontFamily: 'var(--font-playfair), Georgia, serif', color: '#1a3a2a' }}>Destinations halal populaires</h2>
            </div>
            <Link href="/destinations" className="text-sm font-medium hover:underline" style={{ color: '#1a3a2a' }}>Voir tout →</Link>
          </div>
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
            {DESTINATIONS.map((d) => (
              <Link key={d.slug} href={`/destinations/${d.slug}`} className="group block relative overflow-hidden" style={{ borderRadius: '9999px 9999px 1.5rem 1.5rem' }}>
                <div className="relative aspect-[3/4]">
                  <Image src={d.image} alt={d.city} fill className="object-cover group-hover:scale-105 transition-transform duration-500" />
                  <div className="absolute inset-0 bg-gradient-to-t from-black/75 via-black/10 to-transparent" />
                  <div className="absolute top-4 left-1/2 -translate-x-1/2">
                    <span style={{ backgroundColor: 'rgba(201,168,112,0.9)', color: '#1a3a2a' }} className="text-[10px] font-bold px-3 py-1 rounded-full whitespace-nowrap tracking-widest">{d.badge}</span>
                  </div>
                  <div className="absolute bottom-0 left-0 right-0 p-4 text-center">
                    ...
                    <div style={{ color: '#c9a870' }} className="text-xs mt-2 opacity-0 group-hover:opacity-100 transition-opacity">Guide complet →</div>
                  </div>
                </div>
              </Link>
            ))}
      ...
      <section className="py-20 px-4 bg-white">
        <div className="max-w-5xl mx-auto text-center mb-14">
          ...
          <h2 className="text-3xl font-bold" style={{ fontFamily: 'var(--font-playfair), Georgia, serif', color: '#1a3a2a' }}>Voyager Halal, simplifié</h2>
        </div>
      ...
      <section style={{ backgroundColor: '#1a3a2a' }} className="py-20 px-4">
        <div className="max-w-4xl mx-auto flex flex-col lg:flex-row items-center gap-12">
          <div className="flex-1">
            ...
            <h2 className="text-3xl font-bold text-white mb-5" style={{ fontFamily: 'var(--font-playfair), Georgia, serif' }}>Voyages Halal dans votre poche</h2>
            <p className="text-white/50 text-sm leading-relaxed mb-8 max-w-md">Géolocalisation, boussole Qibla, horaires de prière, restaurants proches — tout ce dont vous avez besoin, même sans connexion.</p>
            <Link href="/application" style={{ backgroundColor: '#c9a870', color: '#1a3a2a' }} className="inline-block font-bold text-sm px-8 py-3 rounded-full hover:opacity-90 transition-opacity">En savoir plus</Link>
          </div>
          <div style={{ backgroundColor: '#2d5a3d' }} className="w-48 h-80 rounded-3xl flex items-center justify-center text-6xl shrink-0">📱</div>
        </div>
      </section>
    </main>
  )
